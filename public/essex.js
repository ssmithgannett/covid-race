jQuery.ajax({
  url : 'https://ghminternaldev.wpengine.com/wp-content/plugins/ghm-admin/covid1/essex-milken.php',
  type : 'GET',
...
jQuery.ajax({
  url : 'https://ghminternaldev.wpengine.com/wp-content/plugins/ghm-admin/covid1/essex-usat2.php',
  type : 'GET',
...
function pullData(usat) {
  ...
  for (var i = 0; i < usat.length; i++) {
    ...
    var variable1 = usat[i].no_health_insurance_coverage;

    variable1 = percent(variable1,totalPop);
    var variable2 = usat[i].median_household_income;
    ...
    if (county == 'Essex' && state == 'New Jersey') {
      ...
      //var1 (uninsured)
      jQuery('.var1 .fcounty-context').text(variable1 + ' percent');
   ...
   var enteredCounty = jQuery(this).val();
    ...
    for (var i = 0; i < usat.length; i++) {
      ...
      if (enteredState == state && enteredCounty == county) {
        ...
        //var1 (uninsured)
        jQuery('.var1 .ecounty-context').text(variable1 + ' percent');
...
function pullData2(milken) {
  ...
  for (var x = 0; x < milken.length; x++) {
    ...
    var variable4 = milken[x].percent_homeowners;

    var variable5 = milken[x].residential_segregation_black_white_higher_value_indicate_greater_segregation;
    ...
    if (milkenState == enteredState && milkenCounty == enteredCounty) {
      console.log('!');
        ...
        jQuery('.var5 .ecounty-context').text(variable5);
        ...
        jQuery('.var5 .fcounty-context').text(variable5);
      }
    }
  });///
}
...
jQuery('.explainer').on('click', function(){
  alert('!');
});
