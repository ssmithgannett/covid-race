jQuery.ajax({
  url : 'https://ghminternaldev.wpengine.com/wp-content/plugins/ghm-admin/covid1/imperial-milken.php',
  type : 'GET',
...
jQuery.ajax({
  url : 'https://ghminternaldev.wpengine.com/wp-content/plugins/ghm-admin/covid1/imperial-usat.php',
  type : 'GET',
...
console.log(usat);
  for (var i = 0; i < usat.length; i++) {
    ...
    var variable3 = usat[i].unemployment_rate;
    var variable4 = usat[i].no_health_insurance_coverage;
    variable4 = percent(variable4,totalPop);
    var variable5 = usat[i].percent_below_poverty_level;

    var county = usat[i].county;
    var state = usat[i].state;

    // featured county
    if (county == 'Imperial' && state == 'California') {
      ...
      jQuery('.frace-fcounty span').text('Hispanic population: '+percent(hispanicPop,totalPop));
      ...
      jQuery('.var0 .fcounty-context2').text(percent(hispanicPop,totalPop) + ' percent');

      //var3 (unemployment)
      jQuery('.var3 .fcounty-context').text(variable3 + ' percent');

      //var4 (health insurance)
   ...
   var enteredCounty = jQuery(this).val();
    ...
    for (var i = 0; i < usat.length; i++) {
      ...
      if (enteredState == state && enteredCounty == county) {
        ...
        jQuery('.frace-ecounty span').text('Hispanic population: '+percent(hispanicPop,totalPop));
        ...
        jQuery('.var0 .ecounty-context2').text(percent(hispanicPop,totalPop) + ' percent');


        //var3 (food insecurity)
...
function pullData2(milken) {
  ...
  for (var x = 0; x < milken.length; x++) {
    ...
    var variable1 = milken[x].adult_obesity;
    var variable2 = milken[x].children_eligible_for_free_or_reduced_price_lunch;
    ...
    } else if (milkenCounty == 'Imperial' && milkenState == 'California') {
